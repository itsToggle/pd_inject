"""Module to provide functionalities that can be run as tasks.
...
class mockserver:
    ...
    def register(self):
        """Execute the primary action of this module.
    ...
    def identify(self, path):
        ...
        type = "movie" if 'type=1' in path or "movie" in guid else 'episode' if 'type=4' in path or "episode" in guid else "show"
        guid = guid.split('/')[-1]
        s = [int(regex.search(r'(?<=season.index=)(.*?)(?=&)', path,
                              regex.I).group())] if regex.search(r'(?<=season.index=)(.*?)(?=&)', path, regex.I) else None
        e = None
            ...
            for section_ in response['MediaContainer']['Directory']:
                if section_['key'] in library.refresh.SECTIONS and release['type'] == section_['type']:
                    ...
                    for location in section_['Location']:
                        if library.refresh.PARTIAL:
                            folders += [requests.utils.quote(location['path'] + "/" + release['title'])]
                        else:
